...
function DisplayTweetMedia ({ bords }) {
  ...
  const getTweets = async () => {
    try {
      await fetch(`http://127.0.0.1:8000/api/getTweets/${user.id}`)
  ...
  function goComments (e, id) {
    if (e.target.localName === 'a') return
    else if (e.target.localName === 'span' || e.target.localName === 'svg') return
    navigate('/Home/' + id)
  }
  ...
  const handleLike = (e, id) => {
    ...
    fetch('http://127.0.0.1:8000/api/postLike', {
      method: 'POST',
  ...
  return <>
        {tweets.map(tweet => (
                <div className={`${bords} cursor-pointer`} key={tweet.id} onClick={(e) => goComments(e, tweet.id)}>
                    ...
                    <div className='w-full p-2'>
                        ...
                        <div>
                            <p id={`${tweet.id}`}>{tweet.body ? wrapTags(tweet.body) : ''}</p>
                        ...
                        <div className='flex justify-around pr-6 pt-4 pb-2'>
                            <div onClick={(e) => { handleCom(tweet.id) }}
                            ...
                            <div onClick={(e) => { handleRT(tweet.id) }}
                          ...
                          <form>
                            <div onClick={(e) => { handleLike(tweet.id) }}
